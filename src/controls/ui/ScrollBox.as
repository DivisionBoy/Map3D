package controls.ui {
	...
	public class ScrollBox extends Sprite {
		...
		public function ScrollBox(content:DisplayObject, boxHeight:Number, scrollerWidth:Number = 10, scrollerPimpColor:uint = 0x808080) {
			...
			var _containerShadow:Sprite = createSprite(content.width+offset, _height,0xCCCCCC,"");
		...
		public function updateGraphic(cListHeight:Number):void {
			//trace(cListHeight)
			contentWay = cListHeight - _height;
			pimpHeight = _height - contentWay;
			maxPimpWay = _height - _pimpMinHeight;
			
			//_pimp.y = Math.min(Math.max(0, _pimp.y), _height - _pimp.height)
			_container.y = -_pimp.y * _k;
